...
void cancelHuixian(int fd){
	...
	options.c_cc[VMIN] = 33; //read当读取33个字节才返回
	options.c_cc[VMIN] = 1; //read当读取1个字节就返回
	tcflush(fd, TCIFLUSH);
...
int main()
{
	...
	printf ("\n这是一个串口读写程序！\n");
	int charCount = 0;//字符读取计数。
	int flag=0;//等于1表示该字符是33字节中的一个
	//a,串口相关配置及初始化
	int fd;
	int nread;
	char sendbuff[BUF_SIZE];//供scoket接收数据和向串口写数据使用

	char recvbuff[1];//供串口接收数据和向socket发送数据使用
	char data[BUF_SIZE];//向socket发送数据使用
